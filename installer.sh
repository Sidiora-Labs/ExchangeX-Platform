...
import_initial_sql() {
  print_yellow "Importing db/initial.sql to the database..."
  mysql -u "$DB_USER" -p"$DB_PASSWORD" -h "$DB_HOST" -P "$DB_PORT" "$DB_NAME" < db/initial.sql
  if [ $? -eq 0 ]; then
    print_green "db/initial.sql imported successfully."
  else
    print_red "Failed to import db/initial.sql. Please check the file and your database credentials."
    exit 1
  fi
}
