...
const ExchangeProviderPage = () => {
  ...
  const handleActivateProvider = async () => {
    ...
    if (!error) {
      await fetchExchangeDetails(productId as string);
    }
    setActivating(false);
  };
  const exchangeDetails: Record<string, any> = {
    ...
    okx: {
      imgSrc: "/img/exchanges/okx.svg",
      restrictedCountries:
        "United States, Singapore, Ontario (Canada), North Korea, Iran, Cuba, Syria, Afghanistan, Belarus, Myanmar, Zimbabwe, Iraq, Bangladesh",
      description:
        "OKX is a leading crypto trading app that offers a wide range of financial services to global traders and institutional clients through its proprietary trading technologies. Founded in 2017, OKX provides hundreds of tokens and futures trading pairs to help traders optimize their strategy.",
    },
    xt: {
